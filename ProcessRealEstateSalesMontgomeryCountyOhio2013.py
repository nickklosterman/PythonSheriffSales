...
_Table="RealEstateSalesMontgomeryCountyOhio2013"
...
def InsertIntoDB(row,loginfile): #PARID,PARCELLOCATION,SALETYPE,SALEVALIDITY):
    ...
    with con:
        ...
        cur.execute("INSERT INTO RealEstateSalesMontgomeryCountyOhio2013(SALEDT,PARID,PARCELLOCATION,SALETYPE,SALEVALIDITY,PRICE) VALUES ( %s,%s,%s,%s,%s,%s)", (date, row['PARID'],row['PARCELLOCATION'],saletype,salevalidity,row['PRICE'])) #even though their database types are int/float etc they are entered as strings here.... 
    con.commit()
    cur.close()
    con.close()  
...
def InsertUpdateIfExistsIntoDB(row,loginfile): #date,row): #PARID,PARCELLOCATION,SALETYPE,SALEVALIDITY,PRICE):
    ...
    with con:
        ...
        saletype=convertSALETYPEToChar(row['SALETYPE'])
        salevalidity=convertSALEVALIDITYToChar(row['SALEVALIDITY'])
#        cur.execute("INSERT INTO RealEstateSalesMontgomeryCountyOhio2013(SALEDT,PARID,PARCELLOCATION,SALETYPE,SALEVALIDITY) VALUES ( %s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE PRICE=%s", (date, PARID,PARCELLOCATION,saletype,salevalidity,PRICE)) #even though their database types are int/float etc they are entered as strings here.... 
        cur.execute("INSERT INTO RealEstateSalesMontgomeryCountyOhio2013(SALEDT,PARID,PARCELLOCATION,SALETYPE,SALEVALIDITY) VALUES ( %s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE PRICE=%s", (date, row['PARID'],row['PARCELLOCATION'],saletype,salevalidity,row['PRICE'])) #even though their database types are int/float etc they are entered as strings here.... 
...
#        print("%s-=-%s-=-%s",date,parid,parcellocation)
        ...
        resultcount=int(cur.execute("SELECT id FROM RealEstateSalesMontgomeryCountyOhio2013 WHERE SALEDT=%s and PARID LIKE %s and PARCELLOCATION LIKE %s", (date, parid, parcellocation )))  # look for match on all fields except those that would've been update after teh property was sold
        if resultcount==1:
            row=cur.fetchone()
            key=int(row['id'])
...
#            print("SELECT * FROM RealEstateSalesMontgomeryCountyOhio2013 WHERE SALEDT=%s and PARID=%s and PARCELLOCATION=%s " % (date, parid,parcellocation))
    cur.close()
    con.close()  
...
def DropTableFromDatabase(loginfile):
    ...
    with con:
        cur = con.cursor(mdb.cursors.DictCursor)
        cur.execute("DROP TABLE IF EXISTS RealEstateSalesMontgomeryCountyOhio2013")
    con.commit()
    cur.close()
    con.close()
...
inputfilename="RealEstateSalesData/MontgomeryCountyOhio/SALES_2013_RES.csv"
