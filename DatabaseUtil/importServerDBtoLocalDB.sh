...
if [ 1 -eq 1 ]
then
echo "Dumping from remote database as $dbuserRemote."
mysqldump -u "$dbuserRemote" -p djinnius_SheriffSales -h mysql.server272.com -P 3306 > "$outputfilename"
...
if [ -e "$outputfilename" ]
then 
    echo "Logging into the remote database as $dbuserLocal."
    echo "Look in ~/.mysqllogin for passwords/logins."
    mysql -u "$dbuserLocal" -p SheriffSales < "$outputfilename"
fi
