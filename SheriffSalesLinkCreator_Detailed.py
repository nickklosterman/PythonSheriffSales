...
def ProcessFile(inputfilename,outputfilename):
    print("Using:%s and %s",(inputfilename,outputfilename))
...
def InsertIntoDB(SaleDate,CaseNumber,Address, Plaintiff,Defendant,Attorney,SoldTo,PID,Zipcode,Appraisal,MinBidAmt,SaleAmt,SaleStatus):
...
def GeocodeDatabase():
    ...
    with con:
        ...
        cur.execute("SELECT * FROM Property WHERE Latitude is NULL")
        rows = cur.fetchall()
