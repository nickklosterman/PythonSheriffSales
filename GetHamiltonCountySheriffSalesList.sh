...
txtCaseno=""
txtAddress=""
curl -d "txtAddress=${txtAddress}&txtCaseno=${txtCaseno}&ddlDate=${ddlDate}&ddlTown=${ddlTown}&__EVENTVALIDATION=${eventValidation}&__VIEWSTATEENCRYPTED=${viewStateEncrypted}&__EVENTTARGET=${eventTarget}&__EVENTARGUMENT=${eventArgument}&_TSM_HiddenField=${tsm_HiddenField}__VIEWSTATE=${viewState}&txtAddress_TextBoxWatermarkExtender_ClientState=${txtAddress}&btnCurrent=${btnCurrent}" http://apps.hcso.org/PropertySale.aspx > "${outputFileName}"
